...
BOOST_AUTO_TEST_CASE(SO3) {
  ...
  Eigen::Matrix3d X = SE3.exp_SO3(w);
  Eigen::Matrix3d ref = SE3.skew(w).exp();
...
BOOST_AUTO_TEST_CASE(Adj) {
  ...
  Eigen::MatrixXd X = SE3.exp_SEK3(w);
  Eigen::MatrixXd Adj = SE3.adjoint_SEK3(X);
  ...
  for (int i = 0; i < 4; i++) {
    top = SE3.skew(X.block(0, 3 + i, 3, 1)) * X.block(0, 0, 3, 3);
...
BOOST_AUTO_TEST_CASE(exp) {
  ...
  Skew.topLeftCorner(3, 3) = SE3.skew(w.head(3));
  for (int i = 0; i < 4; i++) {
